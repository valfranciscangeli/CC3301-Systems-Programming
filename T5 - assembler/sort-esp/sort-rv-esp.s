...
.while_begin:           #     // del ciclo para mayor eficiencia
    ...
    lw      a1,4(t0)         # en a0 y a1 tenemos a p[0] y a p[1]
    
    li      a5, 0           # a5 = 48 = '0'
    li      a6, 32           # a6 = 32 = ' '
    
    li      t3, 0  # cntp1 = 0  contador de espacios p[0]
    li      t4, 0   # cntp2 = 0 contador de espacios p[1]

.spaces1: #aca contamos espacios en p1
    lbu     a2 , 0(a0)       #a2 = *p1 = p[0] = *a0
    bne     a2, a6, .notspace1     #si a2 == ' '
    addi    t3, t3, 1        #cntp1++
    
.notspace1:
    addi    a0,a0,1          #a0++    
    bne     a2,a5,.spaces1   # si a2 no es cero, se repite     
    
.spaces2: #aca contamos espacios en p2
    lbu     a2 , 0(a1)       #a2 = *p2 = p[1] = *a1
    bne     a2, a6, .notspace2     #si a2 == ' '
    addi    t4, t4, 1        #cntp2++
    
.notspace2:
    addi    a1,a1,1          #a1++    
    bne     a2,a5,.spaces2   # si a2 no es cero, se repite     

    slt     t1, t4, t3 #     // Dejar resultado de la comparacion en t1
...
.decision:              #     if (0>=rc) {
    ...
    blt     zero,t1,.else
    addi    t0,t0,4     #       p++; // notspacer en arreglo de enteros
    j       .while_cond #     }
